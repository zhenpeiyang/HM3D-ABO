...
FUSION_FUNCTION
inline void fusion_dhw2xyzv2(int d, int h, int w, float min_x, float min_y, float min_z, float vx_size, float& x, float& y, float& z) {
  // +0.5: move vx_center from (0,0,0) to (0.5,0.5,0.5), therefore vx range in [0, 1)
  // *vx_size: scale from [0,vx_resolution) to [0,1)
  // -0.5: move box to center, resolution [-.5,0.5)
  x = ((w + 0.5) * vx_size) + min_x;
  y = ((h + 0.5) * vx_size) + min_y;
  z = ((d + 0.5) * vx_size) + min_z;
}
