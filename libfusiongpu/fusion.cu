...
__global__ void kernel_fusion(int vx_res3, const Views views, const FusionFunctorT functor, float min_x, float min_y, float min_z, float vx_size, Volume vol) {
  CUDA_KERNEL_LOOP(idx, vx_res3) {
    int d,h,w;
    fusion_idx2dhw(idx, vol.width_,vol.height_, d,h,w);
    float x,y,z;
    //fusion_dhw2xyz(d,h,w, vx_size, x,y,z);
    fusion_dhw2xyzv2(d,h,w, min_x, min_y, min_z, vx_size, x,y,z);
...
void fusion_gpu(const Views& views, const FusionFunctorT functor, float min_x, float min_y, float min_z, float vx_size, Volume& vol) {
  Views views_gpu;
  ...
  kernel_fusion<<<GET_BLOCKS(vx_res3), CUDA_NUM_THREADS>>>(
    ...
    vx_res3, views_gpu, functor, min_x, min_y, min_z, vx_size, vol_gpu
  );
...
void fusion_projectionmask_gpu(const Views& views, float min_x, float min_y, float min_z, float vx_size, bool unknown_is_free, Volume& vol) {
  ...
  fusion_gpu(views, functor, min_x, min_y, min_z ,vx_size, vol);
}
